...
public class MovementController {
    ...
    @PostMapping("/create")
    public ResponseEntity<MovementDTO> create(@RequestBody MovementDTO movementDTO) throws GenericException, ExecutionException, InterruptedException {
        // return movementFeignClient.createMovement(movementDTO);
        return ResponseEntity.ok(movementService.create(movementDTO));
    }

    @PutMapping("/update")
    public ResponseEntity<MovementDTO> update(@RequestBody MovementDTO movementDTO) throws GenericException, ExecutionException, InterruptedException {
        // return movementFeignClient.updateMovement(movementDTO);
        return ResponseEntity.ok(movementService.update(movementDTO));
    }

    @GetMapping("/{id}")
    public ResponseEntity<MovementDTO> getById(@PathVariable UUID id) throws GenericException {
